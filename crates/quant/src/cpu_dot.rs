...
/// Full q5_K dequant to f32 for independently hash-bound F017 candidate imports.
///
/// This is the production-layout counterpart to [`dequant_q4_k`].  It rejects
/// partial blocks at the caller boundary by returning an empty vector rather
/// than silently ignoring a tail.
...
mod tests {
    ...
    #[test]
    fn q5_k_full_decoder_is_finite_and_rejects_partial_blocks() {
        let source = (0..512)
            .map(|index| ((index * 29 % 251) as f32 - 125.0) / 64.0)
            .collect::<Vec<_>>();
        let mut packed = Vec::new();
        crate::quantize_row_q5_k(&source, &mut packed);
        let decoded = dequant_q5_k(&packed, source.len());
        assert_eq!(decoded.len(), source.len());
        assert!(decoded.iter().all(|value| value.is_finite()));
        assert!(dequant_q5_k(&packed[..packed.len() - 1], source.len()).is_empty());
        assert!(dequant_q5_k(&packed, source.len() - 1).is_empty());
    }
}
