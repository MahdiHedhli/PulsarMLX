    ...
    impl Model {
        pub fn load(path: &Path) -> Result<Model> {
                    ...
                    .is_none_or(|v| v != "off" && v != "-1")
            {
                ...
                let sufs = if shape.family == Family::K3 {
                    K3_SUF
                } else {
                    MLA_SUF
                };
    ...
    impl State {
        ...
        pub fn with_cache(m: &Model, ctx: u32, cache_bytes: usize) -> Result<State> {
            ...
            let (kvq, kvq_rot) = if kv_ok {
                ...
                if rot
                    && s.qk_dim() > s.head_dim
                    && matches!(s.family, Family::Gqa | Family::Qwen35)
                {
                    eprintln!(
            ...
            let (k_bytes, v_bytes) = match s.family {
                ...
                Family::Dsv4 => (s.n_swa as usize * kv_row(s.head_dim as usize), 4),
                Family::Qwen35 => {
                    let b = s.n_head_kv as usize * ctx as usize * kv_row(s.head_dim as usize);
                    (b, b)
                }
