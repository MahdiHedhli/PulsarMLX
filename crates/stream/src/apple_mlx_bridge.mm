...
int pulsar_mlx_array_matvec(
    ...
    if (context == nullptr || !context_generation_is_current(context) ||
        matrix == nullptr || vector == nullptr || matrix->context != context ||
        vector->context != context || matrix->ownership == nullptr ||
        vector->ownership == nullptr || out_array == nullptr) {
        return set_error(error_buffer, error_capacity,
...
int pulsar_mlx_array_copy_f32(
    ...
    auto *array = reinterpret_cast<MlxArrayObject *>(raw_array);
    if (context == nullptr || !context_generation_is_current(context) ||
        array == nullptr || array->context != context || destination == nullptr ||
        count == 0 || mlx_array_dtype(array->array) != MLX_FLOAT32 ||
