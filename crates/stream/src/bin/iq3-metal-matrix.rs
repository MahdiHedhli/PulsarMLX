...
fn main() {
    eprintln!("iq3-metal-matrix requires macOS and Apple Metal");
    std::process::exit(2);
}
...
fn main() {
    if let Err(error) = run() {
        eprintln!("iq3-metal-matrix: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    use stream::{
        Iq3XxsGemvSpec, MetalBridge, StableSlabAllocator, StableSlabConfig, ZeroingPolicy,
    };
    ...
    fn peak_rss_bytes() -> Result<u64, String> {
        ...
        u64::try_from(unsafe { usage.assume_init() }.ru_maxrss)
            .map_err(|_| "peak RSS was negative".to_owned())
    }
    ...
    fn summary(samples: &[f64]) -> Value {
        ...
        json!({
            ...
            "coefficient_of_variation": if mean == 0.0 { 0.0 } else { standard_deviation / mean },
        })
    }
    ...
    let packed_read_seconds = read_start.elapsed().as_secs_f64();
    let read_start = Instant::now();
    let activation_bytes =
        fs::read(&activation_path).map_err(|error| format!("activation read failed: {error}"))?;
    let activation_read_seconds = read_start.elapsed().as_secs_f64();
    ...
    let slab_copy_seconds = copy_start.elapsed().as_secs_f64();
    ...
    let process_first = bridge.iq3_xxs_gemv(&registration, spec, &activation)?;
    for _ in 0..WARMUPS {
        bridge.iq3_xxs_gemv(&registration, spec, &activation)?;
    }

    let mut total = Vec::with_capacity(MEASURED);
    ...
    let mut output_bits: Option<Vec<u32>> = None;
    for _ in 0..MEASURED {
        ...
        output_bits
            .get_or_insert_with(|| result.output.iter().map(|value| value.to_bits()).collect());
        total.push(result.telemetry.total_seconds);
    ...
    let record = json!({
        ...
        "timing": {
            ...
            "dispatch_preparation": summary(&dispatch),
            "synchronization": summary(&synchronization),
            "kernel": if kernel.len() == MEASURED { summary(&kernel) } else { Value::Null },
        },
