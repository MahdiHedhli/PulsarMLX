...
fn run() -> Result<(), String> {
    ...
    match a.get(1).map(String::as_str){
        ...
        _=>Err("usage: f017-native-bounded-p1 plan-only MANIFEST CATALOG OUT | validate-contract CONTRACT REPO_ROOT | machine-preflight CONTRACT REPO_ROOT | execute MANIFEST CATALOG CHECKPOINT_ROOT AUTHORIZATION STATE_ROOT CONTRACT".into())
    }
}
fn main() {
    if let Err(e) = run() {
