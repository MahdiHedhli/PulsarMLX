...
fn store(seed: u64) -> Store {
    ...
    s.vectors
        .insert("output_norm.weight".into(), rng.values(m.hidden));
    ...
    for layer in 0..m.layer_count {
        ...
        for head in 0..m.heads {
            s.experts.insert(
                (format!("blk.{layer}.attn_k_b.weight"), head),
                Matrix {
                    rows: m.kv_rank,
                    columns: m.qk_nope,
                    values: rng.values(m.kv_rank * m.qk_nope),
                },
            );
            s.experts.insert(
                (format!("blk.{layer}.attn_v_b.weight"), head),
                Matrix {
                    rows: m.value_dim,
                    columns: m.kv_rank,
                    values: rng.values(m.value_dim * m.kv_rank),
                },
            );
        }
        s.vectors.insert(
        ...
        s.vectors.insert(
            ...
            rng.values(m.kv_rank),
        );
        s.vectors
            .insert(format!("blk.{layer}.ffn_norm.weight"), rng.values(m.hidden));
        if layer < m.leading_dense_layers {
            for part in ["gate", "up", "down"] {
                let (r, c) = if part == "down" {
                    (m.hidden, m.dense_ffn)
                } else {
                    (m.dense_ffn, m.hidden)
                };
                matrix(
                    &mut s,
            ...
            s.vectors.insert(
                ...
                rng.values(m.expert_count),
            );
            for part in ["gate", "up", "down"] {
                let (r, c) = if part == "down" {
                    (m.hidden, m.expert_ffn)
                } else {
                    (m.expert_ffn, m.hidden)
                };
                for id in 0..m.expert_count {
                    s.experts.insert(
                        (format!("blk.{layer}.ffn_{part}_exps.weight"), id),
                        Matrix {
                            rows: r,
                            columns: c,
                            values: rng.values(r * c),
                        },
                    );
                }
                matrix(
...
fn a_bounded_run_stops_at_max_output_tokens() {
    ...
    assert_eq!(
        outcome.position_selected_tokens[1],
...
fn incremental_text_holds_back_incomplete_characters() {
    ...
    assert_eq!(
        text.push(1),
        Vec::<u8>::new(),
        "half of a two-byte character must be held back"
    );
...
fn the_emitted_answer_is_exactly_the_generated_tokens() {
    ...
    assert_eq!(
        text, expected,
        "no end summary and no diagnostic may reach the answer stream"
    );
}
