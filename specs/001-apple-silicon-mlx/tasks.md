...
- [ ] T027 [US2] Freeze nonsymmetric inputs, expected values, tensor orientation, dtypes, synchronization rules, and pre-result tolerances for elementwise, matmul, embedding, RMS norm, residual, routing, and Q8_0 cases in `fixtures/mlx/manifest.json`
...
- [ ] T035 [US2] Execute the complete fixture suite and commit exact commands, actual results, oracle identities, tolerances, warnings, exclusions, and independent memory gauges in `docs/validation/mlx-tensor-fixtures.json`
- [ ] T036 [US2] Record Q8_0 support separately by tensor role and by scalar/MLX evidence level in `docs/apple-silicon/COMPATIBILITY.md` and update actual US2 boundaries in `docs/apple-silicon/SESSION_LOG.md`
- [ ] T037 [US2] Re-run the exact workspace gates after the shared quant change and append actual macOS results plus an explicit pending Linux/CUDA evidence status in `docs/validation/mlx-tensor-fixtures.json`
...
- [ ] T038 [US3] Define a deterministic reviewable multi-expert split-shard fixture, generation recipe, expected logical ranges, routes, tie order, weights, output, and license/provenance in `fixtures/mlx/routed-moe-v1.json`
...
- [ ] T042 [P] [US3] Write a failing end-to-end contract test for exact expert payloads and the synthetic routed output in `crates/mlx-backend/tests/synthetic_moe.rs`
...
- [ ] T048 [P] [US3] Execute the synthetic routed-MoE command and commit expert IDs, weights, output parity, memory gauges, actual result, warnings, and synthetic-only exclusions in `docs/validation/synthetic-moe-v1.json`
...
- [ ] T062 [US4] Execute the Apple slice against the same external artifact and prompt, compare only the preselected named output, and commit exact commands, actual errors/result, memory gauges, warnings, and execution-depth exclusions in `docs/validation/qwen3-30b-a3b-q8_0-slice.json`
...
- [ ] T070 [US5] Independently replay one committed validation command and record the reproducer environment, actual result, differences, and exclusions in `docs/validation/reproduction-check.json`
- [ ] T071 [US5] Reconcile verified/planned/unsupported language and executable commands across `README.md`, `specs/001-apple-silicon-mlx/quickstart.md`, `docs/apple-silicon/SESSION_LOG.md`, and `docs/apple-silicon/KNOWN_LIMITATIONS.md`
...
- [ ] T073 [P] Add a lockfile-backed, small-fixture-only Apple MLX job after local US1-US3 evidence exists, keep the exact Cargo baseline job intact, assert `arm64`, and exclude external models in `.github/workflows/macos.yml`
