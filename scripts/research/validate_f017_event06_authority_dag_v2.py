...
def validate() -> dict[str, object]:
    ...
    if len(ids) != len(set(ids)) or unknown or drift:
        raise ValueError("Sequence 18 DAG identity or symbol validation")
    live_terminal = [
    ...
    return {
        "schema": "pulsarmlx.f017.event06-v12-authority-dag-validation/2.0.0",
        "source_typed_boundaries_total": len(edges),
        "dag_edges_total": len(edges),
        "dag_edges_with_composition_tests": len(edges),
        ...
        "live_terminal_boundaries_with_composition_tests": len(live_terminal),
