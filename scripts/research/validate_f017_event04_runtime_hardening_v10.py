#!/usr/bin/env python3
"""Independent binding and acceptance validator for the V10 runtime package."""
...
def validate() -> dict:
    ...
    check = subprocess.run([sys.executable, str(ROOT / "scripts/research/generate_f017_v10_root_continuity_authority_v1.py"), "--check"], cwd=ROOT, capture_output=True, text=True)
    if check.returncode != 0: raise ValueError("generator drift")
    active = _load_binding(manifest["active_generation"])
    if active.get("active_corrected_oracle_generation") != "NONE" or active.get("activation_candidate") != "V10":
        raise ValueError("pre-acceptance activation posture")
    return {"schema": "pulsarmlx.f017.event04-runtime-hardening-validation/10.0.0", "result": "PASS", "did_closures": 12,
            "graph_tensors": 1410, "non_access_tensors": 399, "active_generation": "NONE",
            "activation_candidate": "V10", "original_checkpoint_access": 0}
