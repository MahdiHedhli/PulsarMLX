...
RELEASE = CONTRACTS / "f017-apple-production-serial-f32-equivalence-single-use-release-v3.json"
...
def validate_repo():
    ...
    code=load(CONTRACTS/"f017-apple-production-serial-f32-code-manifest-v2.json")
    ...
    auth=load(CONTRACTS/"f017-apple-production-serial-f32-future-authorization-schema-v2.json")
    inert=load(CONTRACTS/"f017-apple-production-serial-f32-inert-go-fixture-v1.json")
    ...
    return {"status":"F017_APPLE_SERIAL_F32_EXECUTION_READINESS_VALID","tensor_count":40,"package_root_sha256":package_result["package_root_sha256"],"ledger":175,"checkpoint_reads":0,"shard_opens":0,"production_equivalence_executions":0,"determinism_runs":0,"live_go_tokens":0}


def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--repo",action="store_true"); args=parser.parse_args()
    if not args.repo: raise SystemExit("--repo required")
    print(json.dumps(validate_repo(),sort_keys=True,separators=(",", ":")))
