...
def producer_pass(stdout, stderr, stop, operation):
    ...
    try:
        ...
        expected_tests = {'case': 1, 'matrix': 4, 'discrimination': 4, 'convolution': 6, 'cache': 8, 'recurrent': 6, 'dispatch': 6, 'linear': 5, 'layer': 6, 'moe': 5, 'sparse': 4, 'stack': 4, 'sparse-decode': 3, 'stack-decode': 3, 'sanitize': 3, 'quantized': 4, 'topology': 2, 'linear-long': 3, 'offload': 3, 'load-offload': 2, 'quantized-load': 3, 'offload-pulsar': 3, 'slot-store': 6}[operation]
        if len(results) != 1 or strict(lines[-1]) != results[0]:
            return False
        result = results[0]
        if operation == 'linear' and result.get('test_ids') != [
                'test_frozen_reference_and_anchor', 'test_interleaved_actual_caches',
                'test_preentry_identity_and_archive_refusals', 'test_semantic_controls',
                'test_whole_and_partitions']:
            return False
        if operation == 'layer' and result.get('test_ids') != [
                'test_cache_lifecycle_split_run', 'test_compiled_decode_step', 'test_frozen_reference_recomputes',
        ...
        if operation == 'moe' and result.get('test_ids') != [
            ...
            return False
        if operation == 'sparse' and result.get('test_ids') != [
                'test_block_boundaries', 'test_frozen_reference_recomputes', 'test_refusals', 'test_semantic_mutants']:
            return False
        if operation == 'stack' and result.get('test_ids') != [
                'test_frozen_reference_recomputes', 'test_make_cache_on_real_classes', 'test_semantic_mutants', 'test_stack_boundaries']:
            return False
        if operation == 'sparse-decode' and result.get('test_ids') != [
                'test_decode_boundaries', 'test_frozen_reference_recomputes', 'test_semantic_mutants']:
            return False
        if operation == 'stack-decode' and result.get('test_ids') != [
                'test_decode_boundaries', 'test_frozen_reference_recomputes', 'test_semantic_mutants']:
            return False
        if operation == 'sanitize' and result.get('test_ids') != [
                'test_frozen_reference_recomputes', 'test_sanitize_and_strict_load', 'test_semantic_mutants']:
            return False
        if operation == 'quantized' and result.get('test_ids') != [
        ...
        if operation == 'quantized-load' and result.get('test_ids') != ['test_dsv32_quantized_kv_b_split', 'test_quantize_export_reload_and_mixed_layout', 'test_structural_controls']:
            return False
        if operation == 'offload-pulsar' and result.get('test_ids') != ['test_frozen_reference_recomputes', 'test_lfu_store', 'test_structural_controls']:
            return False
        if operation == 'slot-store' and result.get('test_ids') != ['test_coverage_controls_v2', 'test_coverage_v2', 'test_fault_injection', 'test_frozen_reference_recomputes', 'test_slot_store', 'test_structural_controls']:
            return False
...
def main():
    ...
    parser.add_argument('operation', choices=('manifest', 'doctor', 'case', 'matrix', 'discrimination', 'convolution', 'cache', 'recurrent', 'dispatch', 'linear', 'layer', 'moe', 'sparse', 'stack', 'sparse-decode', 'stack-decode', 'sanitize', 'quantized', 'topology', 'linear-long', 'offload', 'load-offload', 'quantized-load', 'offload-pulsar', 'slot-store', 'archive-verify'))
