...
def producer_pass(stdout, stderr, stop, operation):
    ...
    try:
        ...
        expected_tests = {'case': 1, 'matrix': 4, 'discrimination': 4, 'convolution': 6, 'cache': 8, 'recurrent': 6, 'dispatch': 6, 'linear': 5, 'layer': 6, 'moe': 5, 'sparse': 4, 'stack': 4, 'sparse-decode': 3, 'stack-decode': 3, 'sanitize': 3, 'quantized': 4, 'topology': 2, 'linear-long': 3, 'offload': 3, 'load-offload': 2, 'quantized-load': 3, 'offload-pulsar': 3, 'slot-store': 3}[operation]
        ...
        if operation == 'offload-pulsar' and result.get('test_ids') != ['test_frozen_reference_recomputes', 'test_lfu_store', 'test_structural_controls']:
            return False
        if operation == 'slot-store' and result.get('test_ids') != ['test_frozen_reference_recomputes', 'test_slot_store', 'test_structural_controls']:
            return False
