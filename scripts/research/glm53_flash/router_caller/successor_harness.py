...
def run(context, mode, backend):
    ...
    if mode in ('stack', 'stack-decode', 'sanitize', 'topology', 'load-offload', 'quantized-load'):
        ...
        if mode == 'load-offload':
            ...
            offload_source = component(context, 'source', {}, folder='decoder_offload')
            load_source = component(context, 'source', {}, folder='decoder_load_offload')
            load_controls = component(context, 'controls', {}, folder='decoder_load_offload')
            return load_controls.run(context, backend, mx, nn, np, refs, sources, controls, topology_controls, quantized_source, offload_source, load_source)
        if mode == 'topology':
            topology_controls = component(context, 'controls', {}, folder='decoder_topology')
            return topology_controls.run(context, backend, mx, nn, refs, sources, controls)
        if mode == 'sanitize':
    ...
    if mode == 'offload':
        ...
        quantized_controls = component(context, 'controls', {}, folder='decoder_quantized')
        offload_source = component(context, 'source', {}, folder='decoder_offload')
        offload_oracle = component(context, 'oracle', {'quantized': quantized_oracle},
