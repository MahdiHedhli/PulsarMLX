...
def child(args):
    ...
    os.environ["FLASH_BOUNDARY_COMPLETION_PHASE"] = str(phase)
    ...
    if args.mode == "serializer":
        ...
        return 0
    if args.mode == "template-smoke":
        manifest=guard.verify_environment(phase)
        import jinja2
        from jinja2.sandbox import SandboxedEnvironment
        guard.verify_origin(jinja2,phase,manifest)
        env=SandboxedEnvironment(undefined=jinja2.StrictUndefined,loader=None)
        env.globals={}
        output=env.from_string('flash: {{ value }}').render(value=3)
        if output!='flash: 3':raise AssertionError('source-free template smoke')
        print(json.dumps({'event':'result','status':'PASS','output':output,'core_limit':list(resource.getrlimit(resource.RLIMIT_CORE))}),flush=True)
        return 0
    if args.mode in {"template", "template-baseline"}:
        guard.verify_environment(phase)
        pattern = "test_glm53_flash_boundary_completion_baseline.py" if args.mode=="template-baseline" else "test_glm53_flash_boundary_completion_template.py"
    else:
        ...
        if args.mode == "smoke":
            ...
            return 0
        pattern = "test_glm53_flash_boundary_completion_numeric.py"
...
def parent(args):
    ...
    public_fixtures = phase.parent / "repos/PulsarMLX/fixtures/research/glm53-flash-boundary-completion-v1"
    ...
    if not checkpoint.exists():
        ...
        for row in rows:
            ...
            if row['path'].startswith('boundary-completion-v1/') or '/boundary_completion/' in row['path'] or '/boundary-completion-v1/' in row['path'] or 'glm53-flash-boundary-completion-v1/' in row['path'] or 'test_glm53_flash_boundary_completion_' in row['path']:
        ...
        dump(checkpoint,{'generation':digest,'associated_candidate_outputs_observed':False,'files':rows,
                         ...
                         'phase':'boundary-completion-v1','mode_first_admitted':args.mode,
                         ...
                         'numeric_criteria':json.loads((public_fixtures/'numeric.json').read_text())['criteria'] if (public_fixtures/'numeric.json').exists() else None})
    ...
    deadline = 8.0 if args.mode in {"template", "template-smoke", "template-baseline"} else 115.0
    ...
    prefix = {"argv": command, "ticket": ticket, "source_inputs": rows,
              "environment_manifest_sha256": guard.sha(guard.environment_manifest(phase)),
              "wheel_lock_sha256": guard.sha(guard.wheel_lock(phase)),
              "timeout_seconds": 10 if args.mode in {"template", "template-smoke", "template-baseline"} else 120,
...
def main():
    ...
    parser.add_argument("--mode", choices=("serializer", "smoke", "boundaries", "template-smoke", "template-baseline", "template"), required=True)
