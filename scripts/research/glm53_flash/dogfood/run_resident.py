...
NOT part of the successor research harness. Steps:
  ...
  2. mlx_vlm.generate with a bounded prefill chunk; log load time, tokens/s, peak memory, device info, the
...
def main():
    ...
    record = {'model': args.model, 'download': download, 'reap': config.get('reap'), 'n_routed_experts': config['text_config'].get('n_routed_experts'),
              'lazy': args.lazy, 'load_seconds': round(t_load, 1), 'generate_seconds': round(t_gen, 1), 'max_tokens': args.max_tokens,
