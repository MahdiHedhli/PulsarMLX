...
The expert budget is given in bytes with --expert-cache-bytes and capped by --max-expert-cache-bytes (default 60000000000,
the ceiling this entrypoint used to hard-code): verify_artifact refuses a larger budget with EXPERT_CACHE_OVER_BUDGET before
any load, so the reference and serve_offload.py can be pointed at one admitted ceiling with the same flag.
"""
import argparse
import hashlib
import json
...
def build_parser():
    """The argument parser, separate from main() so it can be exercised without importing mlx."""
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument('--offload', required=True); ap.add_argument('--expert-cache-bytes', type=int, required=True); ap.add_argument('--max-expert-cache-bytes', type=int, default=60_000_000_000, help='admitted ceiling; refuse above (default 60000000000)'); ap.add_argument('--coalesce-gap', type=int, required=True); ap.add_argument('--write-mode', choices=('stack', 'per-expert'), required=True)
