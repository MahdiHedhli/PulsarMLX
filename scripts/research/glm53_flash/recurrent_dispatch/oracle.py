...
def validate(case, state):
    ...
    if (any(type(n) is not int for n in (B, T, Hk, Hv, Dk, Dv))
            or not (1 <= B <= 2 and 1 <= T <= 7 and 1 <= Hk <= 2
                    and 1 <= Hv <= 4 and Hv % Hk == 0
                    and ((1 <= Dk <= 4 and 1 <= Dv <= 4) or (Dk in (32, 64) and Dv in (4, 8))))):
...
def ops_transition(case, initial_state=None):
...
def transition(case, initial_state=None, *, kernel_mask=False):
    """Separate masked output policy; recurrence/state is the accepted binary64 reference."""
    result = ops_transition(case, initial_state)
    if kernel_mask and case.get('mask') is not None:
        for b in range(case['B']):
            for t in range(case['T']):
                if not case['mask'][b][t]:
                    result['output'][b][t] = zeros((case['Hv'], case['Dv']))
                    result['traces'][t]['output'][b] = zeros((case['Hv'], case['Dv']))
    return result
