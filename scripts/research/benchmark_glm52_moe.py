#!/usr/bin/env python3
"""Benchmark the complete layer-3 top-8 routed plus shared MoE boundary."""
...
from glm52_layer import EXPERT_WEIGHT_SCALE, N_EXPERT_USED  # noqa: E402
...
FROZEN_EXPERT_IDS = [15, 177, 10, 233, 166, 41, 152, 26]
...
def _cpu_oracle(store: Glm52TensorStore, residual: list[float]) -> tuple[list[float], dict[str, Any]]:
    activation = rms_norm(
        residual,
        load_f32_vector(store, f"blk.{LAYER}.ffn_norm.weight"),
        RMS_EPS,
    )
    logits = matvec_weight(
        store,
    ...
    route = glm_route_real(
        logits,
        bias,
        k=N_EXPERT_USED,
        weight_scale=EXPERT_WEIGHT_SCALE,
    )
    if route["expert_ids"] != FROZEN_EXPERT_IDS:
        raise RuntimeError("CPU oracle route differs from frozen golden top-8")
    aggregate = [0.0] * len(residual)
    for expert, weight in zip(route["expert_ids"], route["weights"], strict=True):
        part = run_expert_swiglu(
            store,
            LAYER,
            expert,
            activation,
    ...
    for index, value in enumerate(shared):
        aggregate[index] += value
    return [a + b for a, b in zip(residual, aggregate, strict=True)], route
...
def _run_once(
    ...
    if len(routes) != 1 or routes[0]["expert_ids"] != FROZEN_EXPERT_IDS:
        raise RuntimeError("MLX path route differs from frozen golden top-8")
    return {
        "decoder_mode": cache.decoder_mode,
        "total_seconds": total_seconds,
        "unattributed_router_activation_aggregation_cleanup_seconds": max(
...
def _summaries(samples: list[dict[str, Any]]) -> dict[str, Any]:
    fields = (
        ...
        "mlx_matvec_seconds",
        "unattributed_router_activation_aggregation_cleanup_seconds",
        "total_seconds",
    )
    return {
...
def benchmark(model: Path) -> dict[str, Any]:
    ...
    if source["source_dirty"]:
        raise RuntimeError("worktree must be clean before the MoE benchmark")
    resource_before = sample_pressure().to_public_dict()
    ...
    try:
        ...
        pass_gate = (
            ...
            and oracle_routes[0] == oracle_routes[1]
        ...
        record = {
            "schema": "pulsarmlx.research.glm52-moe-benchmark",
            ...
            **source,
            ...
            "boundary": {
                ...
                "selected_weights": list(oracle_routes[0]["weights"]),
                "shared_expert": 0,
                "shared_weight": 1.0,
            },
            "protocol": {
            ...
            "cpu_oracle": {
                "implementation": "dedicated scalar top-8 plus shared oracle",
                "imports_mlx": False,
