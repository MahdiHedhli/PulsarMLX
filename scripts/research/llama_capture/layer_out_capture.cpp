// PulsarMLX Feature 005: pinned CPU-only l_out-0 layer output capture helper.
//
...
int main() {
    std::fputs(
        "layer-out capture: unavailable: pinned llama.cpp headers are required\n",
        stderr);
    return 2;
}
...
constexpr std::string_view kTargetName = "l_out-0";
...
int fail(const char * message) {
    std::fprintf(stderr, "layer-out capture: %s\n", message);
    return 2;
}
...
bool parse_arguments(int argc, char ** argv, Arguments & result) {
    if (argc == 2 && (std::strcmp(argv[1], "--help") == 0 ||
        ...
        std::fputs(
            "Usage: pulsarmlx-layer-out-capture --model ABSOLUTE_FILE "
...
int main(int argc, char ** argv) {
    ...
    if (decode_status != 0 || !capture.target_complete ||
        ...
        return fail("l_out-0 cancellation proof is incomplete");
    }
    for (float value : capture.values) {
        if (!std::isfinite(value)) {
            return fail("captured l_out-0 contains a non-finite value");
        }
    }
    if (!rows_are_distinct(capture.values)) {
        return fail("captured l_out-0 rows are identical");
    }
    ...
    std::fputs("layer-out capture: captured one bounded CPU l_out-0 value\n", stdout);
    return 0;
}
