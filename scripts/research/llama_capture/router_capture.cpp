// PulsarMLX Feature 002: pinned CPU-only ffn_norm-0 capture helper.
//
...
int main() {
    std::fputs(
        "router capture: unavailable: pinned llama.cpp headers are required\n",
        stderr);
    return 2;
}
...
constexpr std::string_view kTargetName = "ffn_norm-0";
...
int fail(const char * message) {
    std::fprintf(stderr, "router capture: %s\n", message);
    return 2;
}
...
bool parse_arguments(int argc, char ** argv, Arguments & result) {
    if (argc == 2 && (std::strcmp(argv[1], "--help") == 0 ||
        ...
        std::fputs(
            "Usage: pulsarmlx-router-capture --model ABSOLUTE_FILE "
...
std::string capture_record_json(
    ...
    result += "  \"capture_node\": \"ffn_norm-0\",\n";
    result += "  \"shape\": [2, 2048],\n";
...
int main(int argc, char ** argv) {
    ...
    if (decode_status != 0 || !capture.target_complete ||
        ...
        return fail("ffn_norm-0 cancellation proof is incomplete");
    }
    for (float value : capture.values) {
        if (!std::isfinite(value)) {
            return fail("captured ffn_norm-0 contains a non-finite value");
        }
    }
    if (!rows_are_distinct(capture.values)) {
        return fail("captured ffn_norm-0 rows are identical");
    }
    ...
    std::fputs("router capture: captured one bounded CPU ffn_norm-0 value\n", stdout);
    return 0;
}
