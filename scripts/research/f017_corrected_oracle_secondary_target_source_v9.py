...
def _catalog(candidate: dict) -> tuple[dict, list[dict]]:
    ...
    document = json.loads(raw); schema = document.get("schema") if type(document) is dict else None
    ...
    if schema == "pulsarmlx.f017.corrected-oracle-production-tensor-plan/9.0.0":
        return {"geometry": json.loads(GEOMETRY.read_bytes()), "token": 9703, "position": 0}, document.get("graph_tensors")
...
class SecondaryDescriptorStoreV9:
    ...
    def matrix(self, name: str, rows: int, columns: int):
        return SecondaryRowMatrixV9(self, name, None, rows, columns)
    ...
    def expert(self, name: str, expert: int, rows: int, columns: int):
        ...
        return SecondaryRowMatrixV9(self, name, expert, rows, columns)
...
class SecondaryRowMatrixV9:
    def __init__(self, source: SecondaryDescriptorStoreV9, name: str, expert: int | None, rows: int, columns: int):
...
def source_from_inherited_descriptors(candidate: dict, descriptors: list[dict], file_descriptors: list[int]):
    store = SecondaryDescriptorStoreV9(candidate, descriptors, file_descriptors); store.exercise_format_probes()
