...
def _final_output() -> dict[str, Any]:
    ...
    margin = logits[topk[0]] - logits[topk[1]]
    return _boundary(
        ...
        expected={
            ...
            "argmax": topk[0],
            "margin": margin,
            "margin_sha256": _hash_f64([margin]),
        },
...
def _rust_constants(oracle: dict[str, Any]) -> str:
    ...
    values = {
        ...
        "FINAL_OUTPUT_MARGIN_SHA256": boundaries["final_norm_logits_topk"]["expected"]["margin_sha256"],
    }
    lines = [
    ...
    lines.append(f'pub(crate) const FINAL_OUTPUT_EXPECTED_MARGIN: f64 = {final["margin"]!r};')
