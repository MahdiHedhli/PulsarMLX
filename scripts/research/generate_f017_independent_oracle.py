...
def _rust_constants(oracle: dict[str, Any]) -> str:
    ...
    lines.extend(f'pub(crate) const {name}: &str = "{value}";' for name, value in values.items())
    router = boundaries["router"]["expected"]
    lines.append(f'pub(crate) const ROUTER_EXPECTED_IDS: [u64; 4] = {router["selected_ids"]!r};')
    lines.append(f'pub(crate) const ROUTER_EXPECTED_WEIGHTS: [f64; 4] = {router["weights"]!r};')
    lines.append(f'pub(crate) const ROUTER_EXPECTED_OUTPUT: [f64; 4] = {router["output"]!r};')
