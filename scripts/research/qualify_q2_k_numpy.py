#!/usr/bin/env python3
"""Qualify whole-matrix NumPy Q2_K decoding against the scalar oracle."""
...
from ggml_kquants import dequantize_matrix_q2_k_numpy, dequantize_row_q2_k  # noqa: E402
from qualify_iq2_xxs_numpy import _summary  # noqa: E402

CASES = (
    ("blk.78.ffn_gate_exps.weight", 242),
    ("blk.78.ffn_up_exps.weight", 246),
    ("blk.78.ffn_gate_exps.weight", 28),
    ("blk.78.ffn_up_exps.weight", 48),
)
...
def _scalar(encoded: bytes, rows: int, cols: int) -> np.ndarray:
    return np.asarray(dequantize_row_q2_k(encoded, rows * cols), dtype=np.float32).reshape(rows, cols)
...
def _case(store: Glm52TensorStore, name: str, expert_id: int):
    ...
    if loc.type_id != 10 or len(loc.dims) != 3:
        raise TypeError(f"{name}: expected 3D Q2_K expert tensor")
    ...
    offset = expert_id * encoded_bytes
    start = time.perf_counter()
    encoded = store.pread(name, offset, encoded_bytes)
    ...
    start = time.perf_counter()
    vector = dequantize_matrix_q2_k_numpy(encoded, rows, cols)
    ...
    repeated = dequantize_matrix_q2_k_numpy(encoded, rows, cols)
...
def qualify(model: Path) -> dict:
    ...
    try:
        q2_tensors = sorted(name for name, loc in store.tensors.items() if loc.type_id == 10)
        ...
        store.close()
    if q2_tensors != ["blk.78.ffn_gate_exps.weight", "blk.78.ffn_up_exps.weight"]:
        raise RuntimeError(f"Q2_K checkpoint census changed: {q2_tensors}")
    assert benchmark_input is not None
    encoded, rows, cols = benchmark_input
    vector_fn = lambda: dequantize_matrix_q2_k_numpy(encoded, rows, cols)
    ...
    record = {
        "schema": "pulsarmlx.research.glm52-q2-k-numpy-qualification",
        ...
        **source,
        ...
        "checkpoint_format_census": {
            "q2_k_tensor_names": q2_tensors,
            "limitation": "this checkpoint contains Q2_K only in layer-78 routed gate/up tensors; multi-layer or multi-shard Q2_K cases do not exist",
        },
        "scalar_oracle_imports_mlx": False,
        "cases": cases,
        "benchmark": {
            "tensor": BENCHMARK_CASE[0],
            "expert_id": BENCHMARK_CASE[1],
            "shape_rows_cols": [rows, cols],
