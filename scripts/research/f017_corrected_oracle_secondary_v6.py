#!/usr/bin/env python3
"""V6 authorization wrapper for the independent binary32 numerical core."""
...
from f017_corrected_oracle_authorization_v6 import SECONDARY_ROLE, parse_authorization, sha256_path
...
ORACLE_ID = "F017_INDEPENDENT_ACCELERATED_CROSS_CHECK_V2"
...
NUMERICAL_PATH = ROOT / "scripts/research/f017_corrected_oracle_secondary_numerics_v2.py"
TARGET_SOURCE_PATH = ROOT / "scripts/research/f017_corrected_oracle_secondary_target_source_v6.py"
DECODER_PATH = ROOT / "scripts/research/qualify_f017_quantization_matrix_v1.py"
...
def validate(arguments, installed: bool) -> tuple[object, dict]:
    ...
    report = {
        ...
        "event_id": authority.document["secondary_event_id"],
        "consumer_role": SECONDARY_ROLE,
...
def target(arguments) -> int:
    ...
    source = SecondaryTargetSourceV6(
        authority.document,
        arguments.catalog,
...
def main() -> int:
    ...
    for name in ("validate-authorization-candidate", "validate-installed-authorization", "target"):
        ...
        if name == "target":
            ...
            command.add_argument("--backend", choices=("numpy", "mlx"), default="mlx")
