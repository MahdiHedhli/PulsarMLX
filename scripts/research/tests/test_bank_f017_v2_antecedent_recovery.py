...
class V2RecoveryBankerTests(unittest.TestCase):
    ...
    def test_banked_public_evidence_is_complete_and_ledger_is_append_only(self):
        ...
        self.assertEqual(sum(item["tensor_payload_count"] for item in ledger["events"]), 166)
        ...
        self.assertFalse(recovery[0]["consumed_attempt"])
        real2 = [item for item in ledger["events"] if item["attempt"] == "DPREFIX-REAL-2"]
        self.assertEqual(real2[0]["cumulative_tensor_payloads_after_event"], 139)
