...
RECORD = ROOT / "docs/research/glm52/raw/f016-inference-p1-vectorized-0001.json"
...
class Glm52P1RecordTests(unittest.TestCase):
    def test_golden_identity_backend_and_cache_contract(self) -> None:
        ...
        self.assertEqual(cache["decoded_cache_misses"], 3876)
        ...
        self.assertEqual(cache["storage_bytes_avoided"], 2_105_769_984)
        self.assertEqual(cache["decoded_bytes_avoided"], 11_475_615_744)
        self.assertEqual(
        ...
        self.assertEqual([item["phase"] for item in timings], ["prefill", "decode"])
        self.assertEqual([item["token"] for item in timings], [9703, 21615])
        self.assertTrue(all(len(item["layers"]) == 79 for item in timings))
        cold, warm = (item["cache_delta"] for item in timings)
