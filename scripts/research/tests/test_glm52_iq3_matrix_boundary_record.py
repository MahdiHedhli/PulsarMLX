#!/usr/bin/env python3
"""CI-safe semantic checks for the committed real IQ3 matrix boundary record."""
...
RECORD = ROOT / "docs/research/glm52/raw/f016-iq3-matrix-boundary-0001.json"
...
class Glm52Iq3MatrixBoundaryRecordTests(unittest.TestCase):
    def test_one_read_mlx_gpu_exact_output_contract(self) -> None:
        ...
        self.assertEqual(record["matrix"]["projection"], "down")
        self.assertEqual(record["matrix"]["quantization"], "IQ3_XXS")
        self.assertEqual(record["matrix"]["shape"], [6144, 2048])
        ...
        self.assertEqual(
            record["process_first_vector"]["storage_bytes_read"], 4_816_896
        )
        ...
        for sample in record["samples"]["numpy_vectorized"]:
            ...
            self.assertEqual(sample["storage_bytes_read"], 4_816_896)
        for sample in record["samples"]["scalar_reference"]:
            self.assertEqual(sample["storage_read_count"], 6144)
        subprocess.run(
