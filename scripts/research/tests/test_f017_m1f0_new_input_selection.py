...
class M1F0NewInputSelectionTests(unittest.TestCase):
    def test_complete_real_payload_ledger_reconciles_to_166(self):
        document = LEDGER.build_ledger(ROOT)
        self.assertEqual(document["cumulative_tensor_payloads"], 166)
        self.assertEqual(document["prior_m1f0_scoped_total"], 37)
        self.assertEqual(document["prior_scope_omissions_total"], 8)
        self.assertEqual(sum(event["tensor_payload_count"] for event in document["events"]), 166)
        ...
        shared = [event for event in document["events"] if event["attempt"] == "F017-CANONICAL-SHARED-EXPERT-OUTPUT-RECOVERY-1-ATTEMPT-1"]
        self.assertEqual(len(shared), 1)
        self.assertEqual(shared[0]["tensor_payload_count"], 3)
        self.assertEqual(shared[0]["cumulative_tensor_payloads_after_event"], 166)
