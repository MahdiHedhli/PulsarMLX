...
class F018EvidenceTests(unittest.TestCase):
    ...
    def test_committed_iq2_iq3_moe_record_and_table(self) -> None:
        raw = ROOT / "docs/research/glm52/raw/f018-iq2-iq3-moe-layer3-0001.json"
        table = ROOT / "docs/research/glm52/tables/f018-iq2-iq3-moe-layer3-0001.md"
        record = validate_record(load_unique_json(raw))
        self.assertEqual(record["actual_status"], "passed")
        self.assertTrue(all(sample["direct_iq3"]["matrix_count"] == 8 for sample in record["direct_samples"]))
        subprocess.run(
            [
                sys.executable,
