#!/usr/bin/env python3
"""Exact-bit and malformed-input tests for NumPy IQ2_S decoding."""
...
from iq2_s_dequant import (  # noqa: E402
    dequantize_blocks_iq2_s_numpy,
    dequantize_matrix_iq2_s_numpy,
    dequantize_row_iq2_s,
)


def _block(rng: random.Random, d: float) -> bytes:
    return struct.pack("<e", d) + bytes(rng.randrange(256) for _ in range(80))


class IQ2SNumPyTests(unittest.TestCase):
    def test_randomized_blocks_match_scalar_f32_bits(self) -> None:
        rng = random.Random(0x2212)
        encoded = b"".join(_block(rng, rng.uniform(-3, 3)) for _ in range(89))
        scalar = np.asarray(dequantize_row_iq2_s(encoded), dtype=np.float32)
        vector = dequantize_blocks_iq2_s_numpy(encoded)
        repeated = dequantize_blocks_iq2_s_numpy(encoded)
    ...
    def test_signed_zero_and_row_order(self) -> None:
        ...
        scalar = np.asarray(dequantize_row_iq2_s(encoded), dtype=np.float32)
        matrix = dequantize_matrix_iq2_s_numpy(encoded, 2, 256)
    ...
    def test_malformed_inputs_fail_closed(self) -> None:
        ...
        for encoded in (b"", block[:-1], block + b"\0"):
            with self.assertRaises(ValueError):
                dequantize_blocks_iq2_s_numpy(encoded)
        with self.assertRaises(ValueError):
            dequantize_matrix_iq2_s_numpy(block, 0, 256)
        with self.assertRaises(ValueError):
            dequantize_matrix_iq2_s_numpy(block, 1, 255)
        with self.assertRaises(ValueError):
            dequantize_matrix_iq2_s_numpy(block[:-1], 1, 256)
    ...
    def test_nonfinite_scale_fails_closed(self) -> None:
        ...
        with self.assertRaises(ValueError):
            dequantize_blocks_iq2_s_numpy(bytes(block))
