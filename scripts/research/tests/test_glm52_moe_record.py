#!/usr/bin/env python3
"""CI-safe semantic checks for the real top-8 plus shared MoE record."""
...
RECORD = ROOT / "docs/research/glm52/raw/f016-moe-layer3-0001.json"
EXPERT_IDS = [15, 177, 10, 233, 166, 41, 152, 26]
...
class Glm52MoeRecordTests(unittest.TestCase):
    def test_oracle_route_shared_cache_and_exact_mode_contract(self) -> None:
        record = _load()
        ...
        self.assertEqual(record["boundary"]["selected_expert_ids"], EXPERT_IDS)
        self.assertEqual(record["boundary"]["shared_expert"], 0)
        self.assertEqual(record["boundary"]["shared_weight"], 1.0)
        ...
        for mode, expected_reads in (
            ...
            ("numpy_vectorized", 49168),
        ):
            for sample in record["samples"][mode]:
