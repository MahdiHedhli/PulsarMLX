#!/usr/bin/env python3
"""Exact-bit tests for the opt-in NumPy IQ3_XXS decoder."""
...
class NumpyIq3XxsDecoderTests(unittest.TestCase):
    ...
    def test_randomized_finite_blocks_match_scalar_f32_bits(self) -> None:
        rng = np.random.default_rng(0x495133585853)
        blocks = []
        for _ in range(257):
            scale = np.float16(rng.uniform(-4.0, 4.0))
            payload = rng.integers(0, 256, size=96, dtype=np.uint8).tobytes()
    ...
    def test_matrix_shape_contiguity_and_determinism(self) -> None:
        ...
        encoded = b"".join(
            ...
            + rng.integers(0, 256, size=96, dtype=np.uint8).tobytes()
            for index in range(6)
        )
        first = dequantize_matrix_iq3_xxs_numpy(encoded, rows=2, cols=3 * QK_K)
        second = dequantize_matrix_iq3_xxs_numpy(encoded, rows=2, cols=3 * QK_K)
        ...
        np.testing.assert_array_equal(
            first.reshape(-1).view(np.uint32), scalar_f32_bits(encoded)
        )

    def test_malformed_truncated_overlong_and_nonfinite_fail_closed(self) -> None:
        valid = struct.pack("<e", 1.0) + bytes(96)
        for malformed in (b"", valid[:-1], valid + b"\x00"):
            with self.subTest(length=len(malformed)):
                with self.assertRaises(ValueError):
                    dequantize_blocks_iq3_xxs_numpy(malformed)
        with self.assertRaises(ValueError):
            dequantize_matrix_iq3_xxs_numpy(valid, rows=1, cols=2 * QK_K)
        with self.assertRaises(ValueError):
            dequantize_matrix_iq3_xxs_numpy(valid + valid, rows=1, cols=QK_K)
        for value in (float("inf"), float("-inf"), float("nan")):
            with self.subTest(scale=value):
                with self.assertRaises(ValueError):
                    dequantize_blocks_iq3_xxs_numpy(
                        struct.pack("<e", value) + bytes(96)
                    )
