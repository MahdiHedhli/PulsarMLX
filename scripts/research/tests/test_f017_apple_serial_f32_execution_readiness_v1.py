...
class ReadinessMutationTests(unittest.TestCase):
    ...
    def test_code_manifest_mutations(self):
        self.reject(v.validate_code_manifest,self.code,[lambda d:d["artifacts"].__setitem__(slice(None),[r for r in d["artifacts"] if r["path"]!="crates/quant/src/iq_ref.rs"]),lambda d:d["artifacts"].__setitem__(slice(None),[r for r in d["artifacts"] if r["path"]!="crates/quant/src/cpu_dot_tables.rs"]),lambda d:d["artifacts"].__setitem__(slice(None),[r for r in d["artifacts"] if r["path"]!="scripts/research/f017_apple_serial_f32_capture_wrapper_v2.py"]),lambda d:d.__setitem__("execution_code_head","0"*40),lambda d:d["artifacts"][0].__setitem__("sha256","0"*64)])
