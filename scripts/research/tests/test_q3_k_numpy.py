#!/usr/bin/env python3
"""Exact-bit and malformed-input tests for NumPy Q3_K decoding."""
...
from ggml_kquants import (  # noqa: E402
    dequantize_blocks_q3_k_numpy,
    dequantize_matrix_q3_k_numpy,
    dequantize_row_q3_k,
)


def _block(rng: random.Random, d: float) -> bytes:
    return bytes(rng.randrange(256) for _ in range(32 + 64 + 12)) + struct.pack("<e", d)


class Q3KNumpyTests(unittest.TestCase):
    def test_randomized_blocks_match_scalar_f32_bits(self) -> None:
        rng = random.Random(0x3003)
        encoded = b"".join(_block(rng, rng.uniform(-3, 3)) for _ in range(89))
        scalar = np.asarray(dequantize_row_q3_k(encoded), dtype=np.float32)
        vector = dequantize_blocks_q3_k_numpy(encoded)
        repeated = dequantize_blocks_q3_k_numpy(encoded)
    ...
    def test_signed_zero_and_row_order(self) -> None:
        ...
        scalar = np.asarray(dequantize_row_q3_k(encoded), dtype=np.float32)
        matrix = dequantize_matrix_q3_k_numpy(encoded, 2, 256)
    ...
    def test_malformed_inputs_fail_closed(self) -> None:
        ...
        for encoded in (b"", block[:-1], block + b"\0"):
            with self.assertRaises(ValueError):
                dequantize_blocks_q3_k_numpy(encoded)
        with self.assertRaises(ValueError):
            dequantize_matrix_q3_k_numpy(block, 0, 256)
        with self.assertRaises(ValueError):
            dequantize_matrix_q3_k_numpy(block, 1, 255)
        with self.assertRaises(ValueError):
            dequantize_matrix_q3_k_numpy(block[:-1], 1, 256)

    def test_nonfinite_scale_fails_closed(self) -> None:
        rng = random.Random(4)
        block = bytearray(_block(rng, 1.0))
        block[108:110] = struct.pack("<e", float("nan"))
        with self.assertRaises(ValueError):
            dequantize_blocks_q3_k_numpy(bytes(block))
