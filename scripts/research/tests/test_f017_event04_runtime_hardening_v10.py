...
from f017_descriptor_lease_manager_v10 import acquire_synthetic_leases
from f017_event04_tensor_plan_v9 import build_plan, validate_plan
from f017_runtime_outcome_realizer_v10 import realize
from f017_canonical_serialization_v10 import bank_exclusive
from f017_lifecycle_artifact_v8 import bank_runtime_artifact
from f017_synthetic_checkpoint_v10 import prepare
import execute_f017_corrected_oracle_event_v10 as coordinator
from execute_f017_corrected_oracle_event_v10 import execute_event04, _terminalize
import validate_f017_corrected_oracle_access_v10 as authorizer
from f017_memory_gate_v9 import THRESHOLD_BYTES
from f017_corrected_oracle_authorization_v10 import _memory_gate, parse_candidate_bytes, production_shards
...
def descriptors() -> list[dict]:
    return [{"device": 1, "inode": 1000 + ordinal, "mode": 0o100600, "size": 10 + ordinal,
             ...
             "lease_id": f"LEASE-F017-V10-UNIT-{ordinal}"} for ordinal in range(2, 7)]


def production_receipt(emergency: Path, fallback: Path) -> dict:
    return {"schema": "pulsarmlx.f017.corrected-oracle-installation-receipt/10.0.0", "authority": True,
...
def test_target_sources_have_no_graph_path_open_primitive() -> None:
    for name in ("f017_corrected_oracle_primary_target_source_v10.py", "f017_corrected_oracle_secondary_target_source_v10.py"):
...
def test_future_live_candidate_is_renderable_but_not_authority(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bank_exclusive(readiness, {"schema": "pulsarmlx.f017.event04-execution-readiness-declaration/10.0.0",
                               ...
                               "ACTIVE_CORRECTED_ORACLE_GENERATION": "V10", "accepted_implementation_head": authorizer._implementation_head(),
    ...
    bank_exclusive(approval, {"schema": "pulsarmlx.f017.corrected-oracle-event04-operator-approval/10.0.0",
        "result": "APPROVED_FOR_ONE_EVENT_04", "active_generation": "V10",
...
def test_complete_runtime_import_closure_is_byte_bound(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "scripts/research/f017_canonical_serialization_v10.py" in closure
...
def test_absent_synthetic_root_normalizes_to_value_error() -> None:
    candidate = {"scope": "SYNTHETIC_QUALIFICATION", "live": False,
                 "checkpoint_root": "/definitely/absent/f017-v10-root"}
...
def test_live_candidate_rejects_non_authoritative_shard_census(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    value = {"schema": "pulsarmlx.f017.corrected-full-checkpoint-oracle-access-authorization/10.0.0",
        ...
        "attempts": 1, "retries": 0, "resume": False, "active_generation": "V10", "synthetic_root_manifest_path": None,
