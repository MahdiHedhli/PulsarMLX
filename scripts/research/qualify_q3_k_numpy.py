#!/usr/bin/env python3
"""Qualify whole-matrix NumPy Q3_K decoding against the scalar oracle."""
...
from ggml_kquants import dequantize_matrix_q3_k_numpy, dequantize_row_q3_k  # noqa: E402
from qualify_iq2_xxs_numpy import _summary  # noqa: E402

TENSOR = "blk.78.ffn_down_exps.weight"
EXPERTS = (242, 246, 28, 48)
BENCHMARK_EXPERT = 242
WARMUPS = 3
SAMPLES = 10
...
def _scalar(encoded: bytes, rows: int, cols: int) -> np.ndarray:
    return np.asarray(dequantize_row_q3_k(encoded, rows * cols), dtype=np.float32).reshape(rows, cols)
...
def _case(store: Glm52TensorStore, expert_id: int):
    loc = store.tensors[TENSOR]
    if loc.type_id != 11 or len(loc.dims) != 3:
        raise TypeError(f"{TENSOR}: expected 3D Q3_K expert tensor")
    ...
    start = time.perf_counter()
    encoded = store.pread(TENSOR, expert_id * encoded_bytes, encoded_bytes)
    read_seconds = time.perf_counter() - start
    if len(encoded) != encoded_bytes:
        raise OSError(f"{TENSOR}: truncated expert matrix {expert_id}")
    start = time.perf_counter()
    scalar = _scalar(encoded, rows, cols)
    scalar_seconds = time.perf_counter() - start
    start = time.perf_counter()
    vector = dequantize_matrix_q3_k_numpy(encoded, rows, cols)
    ...
    repeated = dequantize_matrix_q3_k_numpy(encoded, rows, cols)
...
def qualify(model: Path) -> dict:
    ...
    try:
        q3_tensors = sorted(name for name, loc in store.tensors.items() if loc.type_id == 11)
        for expert_id in EXPERTS:
            case, encoded, rows, cols = _case(store, expert_id)
            cases.append(case)
            if expert_id == BENCHMARK_EXPERT:
                benchmark_input = (encoded, rows, cols)
    finally:
        store.close()
    if q3_tensors != [TENSOR]:
        raise RuntimeError(f"Q3_K checkpoint census changed: {q3_tensors}")
    assert benchmark_input is not None
    encoded, rows, cols = benchmark_input
    vector_fn = lambda: dequantize_matrix_q3_k_numpy(encoded, rows, cols)
    ...
    record = {
        "schema": "pulsarmlx.research.glm52-q3-k-numpy-qualification",
        ...
        **source,
        ...
        "checkpoint_format_census": {
            "q3_k_tensor_names": q3_tensors,
            "limitation": "this checkpoint contains Q3_K only in the layer-78 routed down tensor; multi-layer or multi-shard Q3_K cases do not exist",
        },
        "scalar_oracle_imports_mlx": False,
        "cases": cases,
        "benchmark": {
            "tensor": TENSOR,
            "expert_id": BENCHMARK_EXPERT,
            "shape_rows_cols": [rows, cols],
        ...
        "unsupported_interpretations": [
            "complete expert, MoE, layer, full-stack, or token-generation speedup",
