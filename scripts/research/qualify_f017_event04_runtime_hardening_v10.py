#!/usr/bin/env python3
"""Overnight-scale V10 runtime hardening and fault qualification."""
...
from execute_f017_corrected_oracle_event_v10 import execute_synthetic, _terminalize
...
from f017_descriptor_lease_manager_v10 import acquire_synthetic_leases, validate_descriptors
from f017_event04_tensor_plan_v9 import build_plan, validate_plan
from f017_runtime_outcome_realizer_v10 import OUTCOMES_PATH, realize
from f017_synthetic_checkpoint_v10 import FORMATS, prepare
from validate_f017_corrected_oracle_access_v10 import install_rehearsal_candidate, render_rehearsal_candidate
...
def _case(seed: int, mixed: bool, case_id: str) -> dict:
    with tempfile.TemporaryDirectory(prefix="f017-v10-success-") as raw:
...
def _release_faults() -> list[dict]:
    ...
    for case in range(30):
        with tempfile.TemporaryDirectory(prefix="f017-v10-release-") as raw:
...
def _descriptor_mutations() -> list[dict]:
    base = [{"device": 1, "inode": 1000 + ordinal, "mode": 0o100600, "size": 100 + ordinal,
             ...
             "lease_id": f"LEASE-F017-V10-MUTATION-{ordinal}"} for ordinal in range(2, 7)]
...
def _terminal_root_faults() -> list[dict]:
    ...
    for case_id, modeled in (("UNSEARCHABLE_EMERGENCY", False), ("UNSEARCHABLE_STATE_MODELED", True)):
        with tempfile.TemporaryDirectory(prefix="f017-v10-terminal-root-") as raw:
...
def qualify(output: Path) -> dict:
    ...
    result = {"schema": "pulsarmlx.f017.event04-runtime-hardening-qualification/10.0.0", "result": "PASS",
