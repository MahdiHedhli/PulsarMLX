...
class PairwiseInputs:
    ...
    def validate(self) -> None:
        values = (
            ...
            self.reduction_j, self.import_i, self.import_j, *self.row_i,
...
def pairwise_bound_scalar(value: PairwiseInputs) -> float:
    ...
    terms.append(outward(8.0 * U64 * (abs(sigmoid(value.logit_i)) + abs(sigmoid(value.logit_j)) + sum(terms))))
    total = 0.0
    for term in terms:
        total = outward(total + term)
    return total
...
def exact_pair_delta(value: PairwiseInputs, lam: float, residual: Sequence[float], rho_i: float, rho_j: float) -> float:
    before = sigmoid(value.logit_i) - sigmoid(value.logit_j)
    ...
    after = sigmoid(value.logit_i + delta_i) - sigmoid(value.logit_j + delta_j)
...
def stress(sample_count: int = 100_000, seed: int = 170_185_001) -> dict[str, float | int]:
    ...
    for _ in range(sample_count):
        ...
        primary = float(pairwise_bound_primary(value)["B_pair"])
