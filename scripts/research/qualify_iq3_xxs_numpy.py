#!/usr/bin/env python3
"""Qualify the NumPy IQ3_XXS decoder against the scalar reference path."""
...
def _scalar_matrix(encoded: bytes, rows: int, cols: int) -> np.ndarray:
    values = dequantize_row_iq3_xxs(encoded, rows * cols)
...
def _compare_case(
    ...
    if loc.type_id != 18 or len(loc.dims) != 3:
        raise TypeError(f"{name}: expected 3D IQ3_XXS")
    ...
    vector = dequantize_matrix_iq3_xxs_numpy(encoded, rows, cols)
    ...
    repeated = dequantize_matrix_iq3_xxs_numpy(encoded, rows, cols)
    ...
    for row in (0, rows // 2, rows - 1):
        ...
        row_scalar = np.asarray(
            dequantize_row_iq3_xxs(row_encoded, cols), dtype=np.float32
        )
        row_vector = dequantize_matrix_iq3_xxs_numpy(row_encoded, 1, cols)[0]
        row_results.append(
            {
                "row": row,
                "exact_f32_bits": bool(
...
def qualify(model: Path) -> dict[str, Any]:
    ...
    vector_function = lambda: dequantize_matrix_iq3_xxs_numpy(encoded, rows, cols)
    ...
    return {
        "schema": "pulsarmlx.research.glm52-iq3-xxs-numpy-qualification",
