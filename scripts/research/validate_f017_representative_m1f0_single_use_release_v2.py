#!/usr/bin/env python3
"""Checkpoint-free semantic validator for repaired one-shot M1-F0 release v2."""
...
from f017_representative_m1f0_ledger_adapter_v1 import CanonicalLedgerAdapter


ROOT = Path(__file__).resolve().parents[2]
RELEASE = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-representative-m1f0-single-use-execution-release-v2.json"
CANDIDATE = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-representative-m1f0-execution-candidate-v3.json"
EXPECTED_HEAD = "3f52388d2e60be23132e53c9cc3386d03a9db389"
EXPECTED_RELEASE_ID = "F017-REPRESENTATIVE-M1F0-ATTENTION-ROUTE-RECOVERY-1-RELEASE-2"
...
EXPECTED_BINDINGS = {
    ...
    "release_wrapper": "29a6c05d1206c5f97bfcbabdb21b1299d9dbb35ffcba7eaffe1ee11526525fa8",
    "ledger_adapter": "ea0e54569499cd7b3fbd3d07ec107ee51c20a86327b9050ec7070b1218c03198",
    "ledger_adapter_contract": "20a0719fd847254c1c7b9f053b9d9df0d0b380177f4914f62f826149fa91c87f",
    ...
    "pre_attempt_failure": "7c4f9f683c1e82ac010ebb0f87005e482ea53ce62666a0efabb2b1a4af6f5340",
    "ledger_adapter_tests": "71caf8e7aadb49d2db0d98bad9db24b819e9e8634d08760ff30e491689444b2b",
    "wrapper_v2_tests": "aad0e36c4e6403f5240904c907469c577781ee1c8aa055da71e566186e7eb9b6",
}
...
def validate_document(release: dict[str, Any], candidate: dict[str, Any], root: Path | None = None) -> list[str]:
    ...
    req(release.get("schema_version") == "2.0.0", "SCHEMA_VERSION")
    ...
    req(release.get("authoritative_repository") == {
        "branch": "feat/017-real-checkpoint-runner",
        "execution_code_head": EXPECTED_HEAD,
        "head_policy": "EXACT_REPAIRED_EXECUTION_CODE_HEAD_WITH_APPEND_ONLY_CONTROL_PLANE",
        "release_control_plane_is_append_only": True,
    }, "AUTHORITATIVE_HEAD")

    supersedes = release.get("supersedes_without_modifying", {})
    req(supersedes == {
        "release_v1": {
            "path": "specs/017-rust-native-inference-runtime/contracts/f017-representative-m1f0-single-use-execution-release-v1.json",
            "sha256": "914e07f12b968df700aa8868ce0888183f83fd77c9c0684302801f8ad09ca1e4",
            "consumed": False,
            "execution_disposition": "SUPERSEDED_DEFECTIVE_WRAPPER_DO_NOT_EXECUTE",
        },
        "release_v1_independent_approval": {
            "path": "docs/architecture/reviews/evidence/f017-representative-m1f0-single-use-release-v1-independent-approval-v1.json",
            "sha256": "4ea1d3eb5af5ac590173e911e62e22435852d9fbc1bf30fc0088cec3a664a9bf",
        },
        "release_wrapper_v1": {
            "path": "scripts/research/f017_representative_m1f0_release_wrapper_v1.py",
            "sha256": "0cc047cd6208a463124e6b2e382fb0d95baffd4c5d5afbdb03cf8580029a0b67",
        },
    }, "SUPERSEDES")
    ...
    if root is not None:
        for item in supersedes.values():
            path = root / item["path"]
            req(path.is_file() and sha(path) == item["sha256"], "SUPERSEDES_FILE")

    req(release.get("canonical_input") == {
    ...
    req(inventory == candidate.get("attention_payload_inventory", []), "INVENTORY")
    req([item.get("ordinal") for item in inventory] == list(range(9)), "INVENTORY_ORDER")
    req(sum(int(item.get("packed_bytes", -1)) for item in inventory) == 132900864, "PACKED_TOTAL")

    req(release.get("read_contract") == {
    ...
    req(release.get("execution_environment") == {
    ...
    req(set(prohibitions) == required_prohibitions and all(prohibitions.get(key) is True for key in required_prohibitions), "PROHIBITIONS")
    ...
    req(invalidity.get("fail_before_attempt_start_if_any_mismatch") == [
        "EXECUTION_CODE_HEAD", "LEDGER", "LEDGER_ADAPTER", "ACCEPTED_BINDING", "ENVIRONMENT",
        "STORAGE", "INVENTORY", "SHARD_OBJECT", "PRIOR_ATTEMPT", "INTERRUPTED_ATTEMPT",
    ], "INVALIDITY")
    req(all(invalidity.get(key) is False for key in ("automatic_rebase", "automatic_reauthorization", "fallback_identity")), "INVALIDITY_FALLBACK")

    req(release.get("approval_boundary") == {
        ...
        "future_wrapper_token_must_bind_new_independent_approval": True,
        "release_v1_token_authorized": False,
        "future_real_event_authorized_scope": "ONLY_RELEASE_2_EXACT_SHA_AFTER_SEPARATE_INDEPENDENT_APPROVAL",
...
def validate_paths(root: Path = ROOT, release_path: Path = RELEASE) -> list[str]:
    errors = validate_document(load(release_path), load(root / CANDIDATE.relative_to(ROOT)), root)
    try:
        current, observations = CanonicalLedgerAdapter(root=root).read()
        if current != 166 or len(observations) != 2:
            errors.append("LEDGER_ADAPTER_RUNTIME")
    except EventError:
        errors.append("LEDGER_ADAPTER_RUNTIME")
    return errors
