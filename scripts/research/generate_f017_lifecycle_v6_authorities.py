#!/usr/bin/env python3
"""Generate all large lifecycle-v6 authority views from the semantic model."""
...
MANIFEST = CONTRACTS / "f017-corrected-oracle-lifecycle-v6-authority-manifest.json"
...
def derive_binding_views(model: dict[str, Any], obligations: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    registry = {
        "schema": "pulsarmlx.f017.corrected-oracle-lifecycle-identity-registry/6.0.0",
    ...
    for identity in identities:
        ...
        for artifact in artifact_classes:
            ...
            cells[artifact] = {
                ...
                "validator": "validate_f017_lifecycle_semantic_authority_v6.py" if outcomes else None,
                "failure_classification": "LIFECYCLE_BINDING_MISMATCH" if outcomes else None,
            }
        rows.append({"identity": name, "cells": cells})
    matrix = {
        "schema": "pulsarmlx.f017.corrected-oracle-lifecycle-binding-matrix/6.0.0",
        "semantic_model_sha256": canonical_sha256(model),
    ...
    schema_view = {
        "schema": "pulsarmlx.f017.corrected-oracle-artifact-schemas/6.0.0",
        "semantic_model_sha256": canonical_sha256(model),
        "artifacts": schemas,
        "status": "GENERATED_VIEW_NOT_PRIMARY_AUTHORITY",
    }
...
def derive_interface(model: dict[str, Any], schemas: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pinned_context": {
            ...
            for key in document["context_keys"]
        },
        "pinned_limits": {
...
def derive_manifest(model: dict[str, Any], documents: dict[str, dict[str, Any]]) -> dict[str, Any]:
    authorities = {
        "model": {
...
def main() -> int:
    ...
    manifest = derive_manifest(model, documents)
    encoded_manifest = canonical_json_bytes(manifest)
    if args.check:
        if not MANIFEST.exists() or MANIFEST.read_bytes() != encoded_manifest:
            raise SystemExit("generated authority drift: " + str(MANIFEST.relative_to(ROOT)))
    else:
        MANIFEST.write_bytes(encoded_manifest)
    print(canonical_json_bytes({"result": "PASS", "generated": {**{name: canonical_sha256(documents[name]) for name in OUTPUTS}, "manifest": canonical_sha256(manifest)}}).decode("ascii"), end="")
    return 0
