#!/usr/bin/env python3
"""Single-use release wrapper v2 with enforced review-to-approval authority."""
...
VALIDATOR = ROOT / "scripts/research/validate_f017_representative_ffn_composition_single_use_release_v2.py"
RELEASE = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-representative-ffn-composition-single-use-release-v2.json"
APPROVAL = ROOT / "docs/architecture/reviews/evidence/f017-representative-ffn-composition-single-use-release-v2-independent-approval-v1.json"
APPROVAL_CONTRACT = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-representative-ffn-composition-release-v2-approval-contract.json"
SUPERSESSION = ROOT / "docs/architecture/reviews/evidence/f017-representative-ffn-composition-release-v1-execution-authority-supersession-v1.json"
V1_RELEASE = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-representative-ffn-composition-single-use-release-v1.json"
V1_REVIEW = ROOT / "docs/architecture/reviews/evidence/f017-representative-ffn-composition-release-cycle-01-independent-review.json"
...
RELEASE_RELATIVE_PATH = "specs/017-rust-native-inference-runtime/contracts/f017-representative-ffn-composition-single-use-release-v2.json"
WRAPPER_RELATIVE_PATH = "scripts/research/f017_representative_ffn_composition_release_wrapper_v2.py"
REVIEW_PARENT = "docs/architecture/reviews/evidence"
REVIEW_BASENAME = re.compile(r"f017-representative-ffn-composition-release-v2-cycle-[0-9]{2}-independent-review\.json")
...
def fixed_paths(home: Path | None = None) -> dict[str, Path]:
    ...
    release_root = anchor / ".local/share/pulsarmlx/f017/representative-ffn-composition-release-2"
    return {
...
def begin_attempt(paths: dict[str, Path], release_path: Path, approval_path: Path, token_path: Path) -> str:
    ...
    return write_state(paths["state_root"], "attempt-start.json", {
        ...
        "release_review_sha256": approval["release_review_sha256"],
        "reviewed_head": approval["reviewed_head"],
...
def validate_fixed_bindings() -> None:
    for path, identity, label in (
        ...
        (APPROVAL_CONTRACT, APPROVAL_CONTRACT_SHA, "APPROVAL_CONTRACT"),
        (SUPERSESSION, SUPERSESSION_SHA, "V1_SUPERSESSION"),
        (V1_RELEASE, V1_RELEASE_SHA, "V1_RELEASE"),
        (V1_REVIEW, V1_REVIEW_SHA, "V1_REVIEW"),
    ):
...
def validate_release_path(release_path: Path) -> dict[str, Any]:
    ...
    require(release.get("schema_version") == "2.0.0", "RELEASE_VERSION")
...
def git_bytes(commit: str, relative_path: str) -> bytes:
    require(re.fullmatch(r"[0-9a-f]{40}", commit) is not None, "REVIEWED_HEAD_FORMAT")
    result = subprocess.run(["git", "-C", str(ROOT), "show", f"{commit}:{relative_path}"], check=False, capture_output=True)
    require(result.returncode == 0, "REVIEWED_HEAD_OBJECT")
    return result.stdout


def require_review_path(relative_path: str) -> Path:
    candidate = Path(relative_path)
    require(not candidate.is_absolute() and candidate.parent.as_posix() == REVIEW_PARENT, "RELEASE_REVIEW_PATH")
    require(REVIEW_BASENAME.fullmatch(candidate.name) is not None, "RELEASE_REVIEW_BASENAME")
    resolved = ROOT / candidate
    metadata = resolved.lstat()
    require(stat.S_ISREG(metadata.st_mode) and not stat.S_ISLNK(metadata.st_mode), "RELEASE_REVIEW_IDENTITY")
    return resolved


def validate_review_chain(release: dict[str, Any], approval: dict[str, Any]) -> None:
    review_path = require_review_path(approval["release_review_path"])
    review_sha = sha256_path(review_path)
    require(approval["release_review_sha256"] == review_sha, "APPROVAL_RELEASE_REVIEW_SHA")
    review = load(review_path)
    expected_review_keys = {
        "schema", "schema_version", "reviewer_identity", "reviewer_model", "reviewed_head",
        "release_path", "release_sha256", "execution_code_head", "verdict", "blocking_findings",
        "non_blocking_required_findings", "defense_in_depth_findings", "statement",
    }
    require(set(review) == expected_review_keys, "RELEASE_REVIEW_KEYS")
    require(review.get("schema") == "pulsarmlx.f017.representative-ffn-composition-single-use-release-v2-independent-review", "RELEASE_REVIEW_SCHEMA")
    require(review.get("schema_version") == "1.0.0", "RELEASE_REVIEW_VERSION")
    require(review.get("reviewer_identity") == REVIEWER_IDENTITY and review.get("reviewer_model") == REVIEWER_MODEL, "RELEASE_REVIEWER")
    require(approval.get("release_reviewer_identity") == review["reviewer_identity"] == REVIEWER_IDENTITY, "APPROVAL_REVIEWER_IDENTITY")
    require(approval.get("release_reviewer_model") == review["reviewer_model"] == REVIEWER_MODEL, "APPROVAL_REVIEWER_MODEL")
    require(approval.get("reviewed_head") == review.get("reviewed_head"), "APPROVAL_REVIEWED_HEAD")
    require(review.get("release_path") == RELEASE_RELATIVE_PATH, "RELEASE_REVIEW_PATH_BINDING")
    require(review.get("release_sha256") == approval.get("release_sha256") == sha256_path(RELEASE), "RELEASE_REVIEW_RELEASE")
    require(review.get("execution_code_head") == approval.get("execution_code_head") == release.get("authoritative_execution_code_head"), "RELEASE_REVIEW_CODE_HEAD")
    require(review.get("verdict") == "ACCEPT" and review.get("blocking_findings") == [] and review.get("non_blocking_required_findings") == [], "RELEASE_REVIEW_VERDICT")
    require(review.get("statement") == "REPRESENTATIVE FFN COMPOSITION SINGLE-USE RELEASE V2 ACCEPTED FOR SEPARATE APPROVAL", "RELEASE_REVIEW_STATEMENT")
    reviewed_head = approval["reviewed_head"]
    ancestor = subprocess.run(["git", "-C", str(ROOT), "merge-base", "--is-ancestor", reviewed_head, "HEAD"], check=False, capture_output=True)
    require(ancestor.returncode == 0, "REVIEWED_HEAD_NOT_ANCESTOR")
    require(sha256_bytes(git_bytes(reviewed_head, RELEASE_RELATIVE_PATH)) == approval["release_sha256"], "REVIEWED_HEAD_RELEASE_BYTES")
    wrapper_sha = release.get("bindings", {}).get("release_wrapper", {}).get("sha256")
    require(sha256_bytes(git_bytes(reviewed_head, WRAPPER_RELATIVE_PATH)) == wrapper_sha, "REVIEWED_HEAD_WRAPPER_BYTES")
    for name, binding in release.get("bindings", {}).items():
        require(sha256_bytes(git_bytes(reviewed_head, binding["path"])) == binding["sha256"], f"REVIEWED_HEAD_BINDING:{name}")
    committed_review = subprocess.run(["git", "-C", str(ROOT), "show", f"HEAD:{approval['release_review_path']}"], check=False, capture_output=True)
...
def validate_approval_document(release: dict[str, Any], approval: dict[str, Any]) -> None:
    expected_approval_keys = {
        ...
        "release_review_path", "release_review_sha256", "release_reviewer_identity",
        "release_reviewer_model", "approver_identity", "approver_model", "verdict", "statement",
        "approval_does_not_execute", "approval_is_not_token", "real_event_authorized", "ledger",
        "checkpoint_read_budget", "shard_open_budget", "future_ffn_compositions",
        "s1_materializations", "s2_constructions", "stop_boundary",
    }
    ...
    require(approval.get("approver_identity") == APPROVER_IDENTITY and approval.get("approver_model") == APPROVER_MODEL, "APPROVER_IDENTITY")
    require(approval.get("statement") == "REPRESENTATIVE FFN COMPOSITION SINGLE-USE RELEASE V2 APPROVED", "APPROVAL_STATEMENT")
    ...
    require(approval.get("real_event_authorized") is False, "APPROVAL_AUTHORIZATION_STATE")
    require(approval.get("ledger") == 175 and approval.get("checkpoint_read_budget") == 0 and approval.get("shard_open_budget") == 0, "APPROVAL_ACCOUNTING")
    require(approval.get("future_ffn_compositions") == 1 and approval.get("s1_materializations") == 0 and approval.get("s2_constructions") == 0, "APPROVAL_EXECUTION_BOUNDARY")
