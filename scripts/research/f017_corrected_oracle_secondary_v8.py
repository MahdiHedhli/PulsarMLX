#!/usr/bin/env python3
"""V8 secondary consumer wrapper over the unchanged binary32 pure core."""
...
from f017_corrected_oracle_secondary_target_source_v8 import source_from_inherited_descriptor
from f017_descriptor_lease_manager_v8 import validate_descriptors


ROOT = Path(__file__).resolve().parents[2]
NUMERICAL = ROOT / "scripts/research/f017_corrected_oracle_secondary_numerics_v2.py"
ROLE = "SECONDARY"
...
def validate_candidate(path: Path) -> dict:
    ...
    if candidate["secondary_numerical_sha256"] != hashlib.sha256(NUMERICAL.read_bytes()).hexdigest():
        raise ValueError("secondary numerical authority")
    return {"result": "PASS", "candidate_sha256": candidate_sha, "consumer_role": ROLE, "event_id": candidate["secondary_event_id"], "checkpoint_opens": 0, "checkpoint_reads": 0, "state_created": False, "numerical_operations": 0}
...
def execute(candidate: dict, descriptors: list[dict], file_descriptors: list[int]) -> dict:
    ...
    store, document, formats = source_from_inherited_descriptor(file_descriptors)
