...
IMMUTABLE = {
    ...
    # Re-pinned 2026-09-20 to the bytes accepted at 5b39a21a ("simplify Event 06
    # to minimum gate path"), which added the opt-in write-once artifact policy
    # and left the non-write-once path byte-identical. The change is reviewed in
    # docs/architecture/reviews/f017-v11-active-measurement-repair-20260920.md and
    # inventoried by the active V11 measurement (v9), whose record for this path
    # must equal the value below. The pin above it -- both numerical cores, the
    # numerical contract, the result authority and the comparison authority --
