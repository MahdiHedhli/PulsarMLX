#!/usr/bin/env python3
"""Generate the bounded IQ2_S qualification table."""
...
SOURCE = ROOT / "docs/research/glm52/raw/post-f016-iq2-s-numpy-qualification-0001.json"
TABLE = ROOT / "docs/research/glm52/tables/post-f016-iq2-s-numpy-qualification-0001.md"
...
def render(record: dict) -> str:
    ...
    for case in record["cases"]:
        ...
        lines.append(f"| `{case['tensor']}` | {case['expert_id']} | {rows}x{cols} | {case['encoded_bytes']/2**20:.3f} | {case['scalar_decode_seconds']:.6f} | {case['vector_decode_seconds']:.6f} | {str(case['exact_f32_bits']).lower()} |")
    lines.extend(
        [
            ...
            "", f"Layer-8 expert-216 gate median decode-only ratio: **{record['benchmark']['median_decode_speedup']:.2f}x**.",
...
def main() -> int:
    ...
    if args.check:
        if not TABLE.exists() or TABLE.read_text() != rendered:
            raise SystemExit("generated IQ2_S table is stale")
    else:
