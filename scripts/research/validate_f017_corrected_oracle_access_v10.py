...
from f017_canonical_serialization_v10 import bank_exclusive, sha256_bytes, strict_bytes
from f017_corrected_oracle_authorization_v10 import LIVE_KEYS, SCHEMA, parse_candidate, production_shards
from f017_corrected_oracle_primary_v10 import validate_candidate as validate_primary
from f017_corrected_oracle_secondary_v10 import validate_candidate as validate_secondary
...
RUNTIME_MANIFEST = ROOT / "specs/017-rust-native-inference-runtime/contracts/f017-corrected-oracle-event04-runtime-authority-manifest-v10.json"
...
def _validate_operator_authorities(approval: object, readiness: object, readiness_sha: str) -> tuple[dict, dict]:
    ...
    if (type(approval) is not dict or set(approval) != approval_keys
            or approval.get("schema") != "pulsarmlx.f017.corrected-oracle-event04-operator-approval/10.0.0"):
        raise ValueError("operator approval census")
    if approval["result"] != "APPROVED_FOR_ONE_EVENT_04" or approval["active_generation"] != "V10":
        raise ValueError("operator approval posture")
    if (type(readiness) is not dict or set(readiness) != READINESS_KEYS
            or readiness.get("schema") != "pulsarmlx.f017.event04-execution-readiness-declaration/10.0.0"
            ...
            or readiness.get("ACTIVE_CORRECTED_ORACLE_GENERATION") != "V10"):
...
def render_rehearsal_candidate(checkpoint_root: Path, shards: list[dict], catalog_path: Path,
    ...
    candidate = {
        ...
        "authorization_id": f"F017-V10-QUAL-AUTH-{identity_suffix}", "package_attempt_id": f"F017-V10-QUAL-PACKAGE-{identity_suffix}",
        "primary_event_id": f"F017-V10-QUAL-PRIMARY-{identity_suffix}", "secondary_event_id": f"F017-V10-QUAL-SECONDARY-{identity_suffix}",
        ...
        "active_generation": "V10", "synthetic_root_manifest_path": str(manifest_path) if manifest_path else None,
...
def install_rehearsal_candidate(candidate_path: Path, installed_path: Path, receipt_path: Path) -> dict:
    ...
    receipt = {"schema": "pulsarmlx.f017.corrected-oracle-installation-receipt/10.0.0", "authority": False,
...
def validate_installed_rehearsal(installed_path: Path, receipt_path: Path) -> dict:
    ...
    if type(receipt) is not dict or set(receipt) != expected or receipt["schema"] != "pulsarmlx.f017.corrected-oracle-installation-receipt/10.0.0":
...
def render_operator_go_candidate(approval_path: Path, readiness_path: Path, catalog_path: Path, output: Path) -> dict:
    ...
    candidate = {
        ...
        "resume": False, "active_generation": "V10", "synthetic_root_manifest_path": None,
...
def install_operator_go_candidate(candidate_path: Path) -> dict:
    ...
    receipt = {"schema": "pulsarmlx.f017.corrected-oracle-installation-receipt/10.0.0", "authority": True,
    ...
    try:
        ...
        _bank_installation_failure((emergency, terminal_fallback), {
            "schema": "pulsarmlx.f017.corrected-oracle-installation-failure/10.0.0",
