#!/usr/bin/env python3
"""Structurally isolated, descriptor-distributed V10 synthetic checkpoints."""
...
def prepare(root: Path, seed: int, suffix: str, mixed: bool = False) -> tuple[Path, list[dict], Path, Path]:
    ...
    shard_names = [f"synthetic-v10-{ordinal:05d}-of-00006.gguf" for ordinal in range(1, 7)]
    payloads = {name: bytearray() for name in shard_names}
    payloads[shard_names[0]].extend(b"F017-V10-IDENTITY-ONLY")
    ...
    manifest = {"schema": "pulsarmlx.f017.synthetic-root-manifest/9.0.0", "purpose": "SYNTHETIC_QUALIFICATION",
                "production_access": "PROHIBITED", "synthetic_package_id": f"F017-V10-SYNTH-{suffix}",
