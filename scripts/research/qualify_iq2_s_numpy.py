#!/usr/bin/env python3
"""Qualify whole-matrix NumPy IQ2_S decoding against the scalar oracle."""
...
from iq2_s_dequant import dequantize_matrix_iq2_s_numpy, dequantize_row_iq2_s  # noqa: E402
from qualify_iq2_xxs_numpy import _summary  # noqa: E402

CASES = (
    ("blk.8.ffn_gate_exps.weight", 216),
    ("blk.8.ffn_up_exps.weight", 244),
    ("blk.8.ffn_gate_exps.weight", 206),
    ("blk.8.ffn_up_exps.weight", 79),
)
...
def _scalar(encoded: bytes, rows: int, cols: int) -> np.ndarray:
    return np.asarray(dequantize_row_iq2_s(encoded, rows * cols), dtype=np.float32).reshape(rows, cols)
...
def _case(store: Glm52TensorStore, name: str, expert_id: int):
    ...
    if loc.type_id != 22 or len(loc.dims) != 3:
        raise TypeError(f"{name}: expected 3D IQ2_S expert tensor")
    ...
    start = time.perf_counter()
    vector = dequantize_matrix_iq2_s_numpy(encoded, rows, cols)
    ...
    repeated = dequantize_matrix_iq2_s_numpy(encoded, rows, cols)
...
def qualify(model: Path) -> dict:
    ...
    try:
        tensors = sorted(name for name, loc in store.tensors.items() if loc.type_id == 22)
        ...
        store.close()
    expected_tensors = ["blk.8.ffn_gate_exps.weight", "blk.8.ffn_up_exps.weight"]
    if tensors != expected_tensors:
        raise RuntimeError(f"IQ2_S checkpoint census changed: {tensors}")
    assert benchmark_input is not None
    encoded, rows, cols = benchmark_input
    vector_fn = lambda: dequantize_matrix_iq2_s_numpy(encoded, rows, cols)
    ...
    record = {
        "schema": "pulsarmlx.research.glm52-iq2-s-numpy-qualification",
        ...
        **source,
        ...
        "checkpoint_format_census": {
            "iq2_s_tensor_names": tensors,
            "limitation": "this checkpoint contains IQ2_S only in layer-8 routed gate/up tensors; multi-layer or multi-shard IQ2_S cases do not exist",
        },
