...
| GLM-5.2 Rust-native runtime (F017) | 🚧 In progress on feature branches; not merged, not claimed. One token verified on the real checkpoint; multi-position decode and the text CLI qualified synthetically; multi-token generation on the real model has not run — [current state](https://github.com/MahdiHedhli/PulsarMLX/blob/eca4b690bd195289f80152649febe0eca22bdbd1/docs/architecture/f017-native-runtime-status.md) |
