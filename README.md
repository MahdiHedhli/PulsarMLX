...
| GLM-5.2 Rust-native runtime (F017) | 🚧 In progress on feature branches; not merged, not claimed. One token verified on the real checkpoint; multi-position decode and the text CLI qualified synthetically; multi-token generation on the real model has not run — [current state](https://github.com/MahdiHedhli/PulsarMLX/blob/c1f0c6a6dffcce8b43d271dee5109120c0ed1772/docs/architecture/f017-native-runtime-status.md) |
...
The shipping architecture for GLM-5.2 is a Rust-native runtime with no required Python hot path ([strategy](docs/roadmap/PULSARMLX_STRATEGY.md)). Its work lives on `feat/017-rust-native-inference-runtime`. State on 2026-09-20: the bounded one-token native executor (Rust + MLX bridge) **executed the real checkpoint and produced token 154820 for prompt 9703, equal to the corrected full-checkpoint oracle** (Event 06), under a human-approved one-shot (attempt 2; attempt 1 on 2026-08-22 had failed against an expected token later proven to come from a defective decoder family). Before the run, the native decoders were shown bit-identical to that oracle's independent decoders on all 11 checkpoint formats and the native graph matched it on the synthetic differential family. One token took 1003.6 s (704 s of shard identity rehash over ~238 GB plus a 299 s forward pass) — a correctness event, not a performance figure. Since that run the track has added the pieces the one-token executor deliberately did not have. `execute_one_token` is position zero with a single visible key — its softmax has one element and RoPE is the identity there — so looping it is not autoregressive generation. A successor module now implements rotary embedding at nonzero positions, causal multi-key attention over the absorbed MLA latent, and a retained per-layer cache with append order, reset, bounds and rollback; it agrees with an independent binary64 reference on 6/6 synthetic seeds (logits max abs ≤ 1.5×10⁻⁷ against a frozen 6.5×10⁻³ threshold, with the selected token, expert selection and order, visible-key count and state growth exact), and a test requires it to reproduce the one-token producer bit for bit at position 0. A native CLI (`f017-native-generate`) takes text and produces text using the checkpoint's own GGUF tokenizer and GLM chat template, with no Python inference process. On 2026-09-20 both ran on the **real 222 GiB checkpoint** under a human approval. A teacher-forced eight-position ladder reproduced the banked one-token receipt exactly at position 0 — token **154820** and full-logits digest `db1456d8…`, identical to the consumed one-shot — with retained state growing to exactly 8 × 182,016 bytes, the figure the contract predicts. A text run then produced `17 times 6 equals` → **` 17 times table`** with no Python in the inference path. What is still **not** claimed: positions after 0 are measured, not qualified, because no independent multi-token oracle exists for this checkpoint; answer quality is not claimed, because four tokens from a 2-bit quantisation is not a task result; and the measured 0.0116 tok/s is this build's cold, uncached weight path, not a runtime capability. The sparse `glm-dsa` indexer is unimplemented, so the runtime refuses sequences longer than the checkpoint's `attention.indexer.top_k` rather than silently substituting dense attention. Every real-checkpoint run is human-gated; formal feature closeout requires a human approval that has not been granted. See the [attempt-2 readiness and outcome note](https://github.com/MahdiHedhli/PulsarMLX/blob/e38d002e54b5552966ccb7a586c2b0cc23b47e08/docs/architecture/reviews/f017-native-attempt-2-readiness-20260920.md). Current state, with every number generated from the evidence records rather than transcribed: [`f017-native-runtime-status.md`](https://github.com/MahdiHedhli/PulsarMLX/blob/c1f0c6a6dffcce8b43d271dee5109120c0ed1772/docs/architecture/f017-native-runtime-status.md) and its machine-readable form [`native-runtime-summary.json`](https://github.com/MahdiHedhli/PulsarMLX/blob/c1f0c6a6dffcce8b43d271dee5109120c0ed1772/docs/glm52-native/native-runtime-summary.json), pinned to `c1f0c6a6` on the feature branch. No native GLM-5.2 execution is committed on the default branch. The historical Python/NumPy research ladder above is banked evidence, not the native runtime.
