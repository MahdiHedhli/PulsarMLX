...
| Verified PulsarMLX capabilities | On the recorded M1 Ultra environment, the final US5 workspace gate listed 172 Rust tests: 171 active tests passed, one native MLX smoke test was explicitly ignored by the general workspace run, and zero failed. At T060 the Python worker discovery suite passed 44 tests. Native MLX 0.32.0 evidence covers an evaluated device proof, seven bounded tensor fixtures, strict Q8_0 reference operations, a portable exact positional source, one synthetic f32 routed-MoE graph, and one evaluated Qwen3MoE Q8_0 gate-projection prefix with zero comparison mismatches and no fallback. |
...
reference](docs/validation/models/qwen3-30b-a3b-q8_0-reference-result.json),
[bounded Apple Qwen result](docs/validation/qwen3-30b-a3b-q8_0-slice.json),
[benchmark decision](docs/validation/benchmark-initial.json), and [known
limitations](docs/apple-silicon/KNOWN_LIMITATIONS.md) for the exact boundaries.
...
Run the committed non-model validation slices on a prepared native MLX host:
...
The external-model command and its mandatory identity, memory, clean-worktree,
and no-fallback stop conditions are in the [Apple MLX validation
quickstart](specs/001-apple-silicon-mlx/quickstart.md). Model files remain
outside Git.
