...
| State | Status |
| --- | --- |
| Multi-token generation on the real checkpoint | **Not run.** Needs a human GO for real-checkpoint access; the request packet is prepared and source-locked. |
| Performance baseline | **Not established.** The only real-checkpoint timing on record is one cold forward pass. No tokens-per-second figure exists and none is published. |
