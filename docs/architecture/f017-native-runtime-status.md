...
| Active source measurement / CI | ✅ | The V11 implementation measurement is repaired: v8 stays frozen and is re-verified against its own head, and an append-only v9 inventories the current head. 28 of 36 measured bodies are unchanged; 8 drifted across accepted commits and are reviewed — the qualify-line integration of 2026-09-20 moved four of them, recorded in the addendum to `reviews/f017-v11-active-measurement-repair-20260920.md`. |
...
| Performance baseline | **First real numbers, not a baseline.** Stage A: 716.6 s of checkpoint identity verification once per session, then 131.2 s for position 0 and 91.7-96.8 s for each of positions 1-7, 1501 s wall for the whole run. These are lower than attempt 2's 299.2 s forward pass because attempt 2 additionally banked 79 per-layer diagnostic records; the arithmetic is the same. A teacher-forced ladder is not a decode rate, so Stage A publishes none. Stages B1 and B2 do generate, and their **measured** rates are in the generated figures block below — 0.0116 and 0.0112 tok/s. Both are measured on a cold, uncached weight path and neither is a runtime capability: what stays unclaimed is *tokens/sec as a runtime capability*, not the existence of a measurement. |
...
**Known defect (2026-09-20, found by Stage B1):** `--no-chat-template`
currently supplies an empty stop set, so a raw-text run always terminates on
`max-tokens` rather than on the model's own terminal. The tokenizer's
end-of-generation ids should apply whether or not the chat template is used.
Chat-template runs are unaffected — they carry the full stop set.
...
| Figure | Value |
| --- | --- |
| Measured bodies unchanged / drifted | 28 / 8 of 36 |
| Produced token, real checkpoint (attempt 2) | 154820 |
| Decoder differential values per seed | 107,502 |
| Decoder differential max ULP | 0 |
| Temporal graph cases (synthetic) | 6/6 |
| Stage A positions, real checkpoint | 8 |
| Stage A later positions | MEASURED_NOT_QUALIFIED |
| Stage B1 generated tokens | 4 |
| Stage B2 positions / prompt tokens | 24 / 21 |
| Measured decode rate, cold uncached weight path | 0.0116 tok/s (Stage B1), 0.0112 tok/s (Stage B2) — **measured, not a runtime capability** |
