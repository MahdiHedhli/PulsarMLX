...
The runtime source is `291295665896c8a489c1f4e5741b199cf5515b2f`.
...
MLX adapter with one conceptual projection, ten deterministic native repeats,
no scaffold/reference/fallback production dispatch, and reconciled teardown.
...
2. run the independent local reference preparer once, reading only the reviewed
   Q8_0 range and exclusively freezing its oracle/package before candidate work;
3. run the literal `--real-projection-boundary` command from the handoff once;
4. bank evidence and stop.
