...
> **IQ3_XXS decoder amendment (2026-08-12):** rows marked `superseded`
> below depended directly or transitively on the pre-v2 Python IQ3_XXS
> decoder, which interleaved the two four-lane grids inside each eight-value
> subgroup instead of preserving canonical GGML order. Their raw records are
> retained as historical observations of that implementation, but they are no
> longer admissible as canonical checkpoint-decoding, numerical-correctness,
> or performance evidence. IQ2-only rows remain verified.
