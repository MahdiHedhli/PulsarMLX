...
latency, throughput, repeatability, memory use, thermal behavior, or
compatibility. Their sidecars and clean-checkout hashes prove generator
provenance and deterministic rendering only. No public raw result package yet
exists under `docs/research/raw/002-router-parity/`.
...
- Protocol v1 has no legal production exclusion rule. The excluded synthetic
  mutation is an expected validator rejection, not an omitted observation.
...
The fixture package cannot establish checkpoint availability or admission,
...
- A raw record cannot promote itself. Package-level validation, committed and
  indexed artifacts, and matching clean-process clean-checkout reproductions
  remain necessary before a ledger claim can be `verified`.
